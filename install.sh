...
CURRENT=$(pwd)
cd /usr/bin
sudo ln -s "$CURRENT/RPi-chromium/youtube-dl/youtube_dl/__main__.py" youtube-dl
cat > update-youtube.sh << EOF
#!/bin/sh
file=__main__.py
maxsize=1000
if [ -d "$CURRENT/RPi-chromium/youtube-dl" ]
then
cd "$CURRENT/RPi-chromium/youtube-dl"
git pull
cd youtube_dl
...
EOF
sudo ln -s "$CURRENT/RPi-chromium/native/run_omxplayer.py" /usr/bin/run_omxplayer.py
sudo mkdir /etc/chromium-browser/native-messaging-hosts
sudo cp "$CURRENT/RPi-chromium/native/run_omx.json" /etc/chromium-browser/native-messaging-hosts/run_omx.json
